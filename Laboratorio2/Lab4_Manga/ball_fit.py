...
def fit_hill_climbing():
    ...
    def neighbors(theta):
        ...
        for i in range(num_neighbors):
            point = np.array([delta*cos(2*pi*i/num_neighbors)+theta[0], delta*sin(2*pi*i/num_neighbors)+theta[1]])
            neighbors_list.append(point)
...
def fit_simulated_annealing():
    ...
    def random_neighbor(theta):
        ...
        return np.array([delta*cos(angle) + theta[0], delta*sin(angle) + theta[1]])
    ...
    def schedule(i):
        ...
        return temperature0/(1 + beta*i*i)
